...
class RulesEngine<T extends BusinessRule> {
  ...
  RulesEngine(this._ruleRepository) : _processors = [
  ...
  Future<RuleExecutionResult> processRules(Map<String, dynamic> context) async {
    ...
    // Ordena as regras por prioridade (maior prioridade primeiro)
    applicableRules.sort((a, b) => b.priority.value.compareTo(a.priority.value));
    ...
    for (final rule in applicableRules) {
      try {
        ...
        if (processor != null) {
          ...
          // Atualiza o contexto com os resultados da regra
