...
/// Implementa o princípio DRY para cálculos comuns de preços e descontos
mixin CalculatorMixin {
  ...
  double calculateTax(double baseValue, double taxRate) {
    if (taxRate < 0) {
      throw ArgumentError('Taxa de imposto não pode ser negativa');
    }
    return baseValue * (taxRate / 100);
  }
  
  /// Calcula o valor com imposto incluído
  double calculatePriceWithTax(double basePrice, double taxRate) {
    return basePrice + calculateTax(basePrice, taxRate);
  }
  
  /// Calcula desconto escalonado (diferentes percentuais para diferentes faixas)
  double calculateTieredDiscount(double originalPrice, int quantity, List<TieredDiscountRule> rules) {
    double totalDiscount = 0;
    
    for (final rule in rules) {
      if (quantity >= rule.minQuantity && (rule.maxQuantity == null || quantity <= rule.maxQuantity!)) {
        final applicableQuantity = rule.maxQuantity != null 
          ? (quantity - rule.minQuantity + 1).clamp(0, rule.maxQuantity! - rule.minQuantity + 1)
          : quantity - rule.minQuantity + 1;
        
        final discountForTier = (originalPrice / quantity) * applicableQuantity * (rule.discountPercentage / 100);
        totalDiscount += discountForTier;
      }
    }
